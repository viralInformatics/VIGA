...
-p PREFIX    prefix directory under which to install VIGA
             [default: current directory]
-b BRANCH    install specified branch of VIGA${BRANCH:+
             [default: $BRANCH]}
-r RELEASE   install specified release package of VIGA${RELEASE:+
             [default: $RELEASE]}
...
title 'installing VIGA'

if [[ -z "${RELEASE}" ]]; then
	message 'Using branch:' "${BRANCH}"

	check_directory 'VIGA' 'VIGA installation directory'
	git clone --depth 1 --branch "${BRANCH}" https://github.com/cbg-ethz/VIGA.git || fail "I cannot install branch ${BRANCH}."
else
	message 'Using release:' "${RELEASE}"
	check_directory "VIGA-${RELEASE}" 'VIGA installation directory'
	${DOWNLOAD} "https://github.com/cbg-ethz/VIGA/archive/refs/tags/${RELEASE}.tar.gz" || fail "I cannot download package {RELEASE}."
...
INIT="$(pwd)/VIGA${RELEASE:+-${RELEASE}}/init_project.sh"

if [[ -z "${WORKDIR}" ]]; then
	status 'Installation of VIGA completed'
	echo "
